...
2. 操作
   - 左スワイプ / ✕ … 見送り（既読にする）
   - 右スワイプ / ♥ … 気になる（既読＋保存）
   - アブストは上下スクロールで読む。「本文を翻訳」をタップするとその場で日本語訳を取得でき、以降は「原文を見る」⇔「日本語訳を見る」で何度でも見比べられる（詳細は後述）
   - 中央の「arXiv」ボタンで abs を開く（Safari のページ翻訳で日本語アブストを読める）
   - 「元に戻す」で直前のスワイプを取り消し（連打すれば同じセッション内で何段階でも戻れる）
   - 右上のリストから保存一覧を表示し、Markdown / BibTeX でコピー、またはバックアップの書き出し/読み込み（詳細は後述）
   - ヘッダー付近に当月の DeepL API 使用量を控えめに表示（80%を超えると警告色）
...
`.github/workflows/update.yml` は `ga_fetch.py --usage-out usage.json` で当月の使用文字数・上限を取得し、`latest.json` と同じタイミングでリポジトリへコミットする。DeepL APIキーはこのワークフロー内（GitHub Secrets経由）でのみ使用され、`usage.json` 自体にはキーは含まれない。アプリはヘッダー付近に「DeepL API使用量: 12,345 / 500,000字（今月）」のように表示し、80%を超えると警告色になる。
...
一方、**古いショートカットを先に削除してしまうと、その保存領域には通常の方法でアクセスできなくなる**（Safariの通常タブで同じURLを開いても別の保存領域になり、データは復元できない）。アイコン更新時は、新しいショートカットの動作を確認できるまで古いショートカットを残しておくか、作り直す前に下記の「バックアップの書き出し/読み込み」で保存リストと既読状態を控えておくこと。

### バックアップの書き出し/読み込み

右上のリスト画面に「バックアップを書き出す」「バックアップを読み込む」ボタンがある。

- **書き出す**：保存リスト（kept）と既読ID（read）をJSONファイルとしてダウンロードする。Markdown/BibTeXコピー（他ツール連携用の書き出し専用フォーマット）とは別物で、こちらはアプリへの読み込み直しを前提にしている。
- **読み込む**：書き出したJSONファイルを選択すると、その内容を**現在のデータに追加マージ**する（上書きはしない。同じ論文IDは重複追加されないので、同じファイルを何度読み込んでも安全）。

アイコンを作り直した直後など、保存領域が空の新しいショートカットでバックアップJSONを読み込めば、以前の保存リスト・既読状態を復元できる。
